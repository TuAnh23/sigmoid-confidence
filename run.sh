...
MODEL_CONFIG=$5  # "configs/model.yaml"
WANDB_RUN_ID=$6  # Can pass in a pre-existing ID to continue training from checkpoint
declare -a TEST_GENERATE_DATA_CONFIGS=( "configs/paracrawl_data.yaml" "configs/wmt24_data.yaml" ) # "configs/pawsx.yaml")
declare -a TEST_SCORE_DATA_CONFIGS=( "configs/wmt24_enzh_FD_bestmt.yaml" "configs/wmt24_enzh_FD_worstmt.yaml" "configs/wmt24_enzh_FD_allmt.yaml" "configs/wmt24_ende_FD_bestmt.yaml" "configs/wmt24_ende_FD_worstmt.yaml" "configs/wmt24_ende_FD_allmt.yaml" "configs/wmt22_general_MT_ende_DA_bestmt.yaml" "configs/wmt22_general_MT_ende_DA_worstmt.yaml" "configs/pawsx.yaml" ) #"configs/pawsx.yaml" )
...
if [[ ${NR_GPUS} == 1 ]]; then
    ...
    python train_sigmoid_head.py \
        --config-file-path ${TRAIN_CONFIG} ${BATCH_CONFIG} ${TRAIN_DATA_CONFIG} ${MODEL_CONFIG} \
        --wandb-run-id ${WANDB_RUN_ID}

    for TEST_GENERATE_DATA_CONFIG in "${TEST_GENERATE_DATA_CONFIGS[@]}"; do
        echo "Inference starts..."
        python inference_sigmoid_head.py \
            --config-file-path "configs/generate_and_eval.yaml" ${BATCH_CONFIG} ${TEST_GENERATE_DATA_CONFIG} ${MODEL_CONFIG} \
            --wandb-run-id ${WANDB_RUN_ID}
    ...
    for TEST_DATA_CONFIG in "${TEST_DATA_CONFIGS[@]}"; do
        ...
        inference_sigmoid_head.py \
            --config-file-path "configs/generate_and_eval.yaml" ${BATCH_CONFIG} ${TEST_DATA_CONFIG} ${MODEL_CONFIG} \
            --wandb-run-id ${WANDB_RUN_ID}
